...
//~					M	Y		C	H	A	N	G	E	S
/*
* 	The following function determines the current situation of congestin
& 	SRTT_S < T (−1) -> low congestion
& 	T (-1) <= SRTT_S < T (+1) -> normal oprating point
& 	T (+1) <= SRTT_S < T (+2) -> medium variability
& 	SRTT_S >= T (+2) -> high variability
*/

double Decision_Boundary (int gamma, double srtt_l, double rttvar_l){
	return srtt_l + gamma*rttvar_l;
}

//*	Funtion to compare rate
double CompRate(double R, double srtt_l , int speed ){
	double w , d;
	if (speed == 2)
		w = 0.2;
	else if (speed == 1)
	...
	if (tcph->seqno() > last_ack_) {
		...
		if(tcph->seqno() >= v_begseq_) {
			...
			if(rtt>0) {
				...
				double r = rtt;    			//* r is latest RTT sample
				double alpha_s = 0.999;		//* alpha_s & alpha_l are smoothing factor
				double alpha_l = 0.001;		//* alpha_s >> alpha_l
				double beta_l = 0.8;
				...
				srtt_s = alpha_s*r + (1-alpha_s)*srtt_s;					//* applying exponential smoothing
				srtt_l = alpha_l*r + (1-alpha_l)*srtt_s;
				rttvar_l = beta_l*abs(srtt_l-r) + (1-beta_l)*rttvar_l;  	//*RTT variance is a measure of 
																			//*the stability of the network delay
																			//*and is calculated by tracking the variation
																			//*of the measured RTT values over time
				
//&					U	P	D	A	T	I	N	G		R	A	T	E

				if(srtt_s < Decision_Boundary(-1, srtt_l, rttvar_l))
					v_actual_ += CompRate(v_actual_, srtt_l, 2);

				else if(Decision_Boundary(-1, srtt_l, rttvar_l) <= srtt_s && srtt_s < Decision_Boundary(1, srtt_l, rttvar_l))
					v_actual_ += CompRate(v_actual_, srtt_l, 1);
				
				else if(Decision_Boundary(1, srtt_l, rttvar_l) <= srtt_s && srtt_s < Decision_Boundary(2, srtt_l, rttvar_l))
					v_actual_ -= CompRate(v_actual_, srtt_l, 1);
				
				if(srtt_s >= Decision_Boundary(2, srtt_l, rttvar_l))
					v_actual_ -= CompRate(v_actual_, srtt_l, 2);
